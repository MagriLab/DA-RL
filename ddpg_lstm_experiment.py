...
_CONFIG = config_flags.DEFINE_config_file(
    "config", "configs/enKF_lstm_config.py", "Contains configs to run the experiment"
)
...
def run_experiment(config, env, agent, wandb_run=None, logs=None, checkpoint_dir=None):
    ...
    # initialize networks
    obs_seq_0 = jnp.zeros(
        (config.train.batch_size, config.train.sequence_steps, env.num_observations)
    )
    action_seq_0 = jnp.zeros(
        (config.train.batch_size, config.train.sequence_steps, env.action_size)
    )

    actor_state, critic_state = agent.initial_network_state(
        key_network, obs_seq_0, action_seq_0
    )
    ...
    def act_observe_and_forecast(
        true_state, true_obs, lstm_carry, params, wait_steps, episode_steps, key_obs
    ):
        ...
        def body_fun(carry, _):
            ...
            true_state, true_obs, lstm_carry, key_obs = carry
            ...
            # get action
            # need to pass the lstm carry throughout!
            action, lstm_carry = agent.actor.apply(
                params, jnp.expand_dims(obs, axis=1), lstm_carry
            )
            ...
            return (true_state, true_obs, lstm_carry, key_obs), (
                true_state_arr,
                true_obs_arr,
                obs,
                action_arr,
        ...
        (true_state, true_obs, lstm_carry, key_obs), (
            true_state_arr,
            true_obs_arr,
            obs_arr,
            action_arr,
            ...
            body_fun, (true_state, true_obs, lstm_carry, key_obs), jnp.arange(n_loops)
        )
        return (
            true_state,
            true_obs,
    ...
    def learn_observe_and_forecast(
        ...
        def forecast_fun(carry, _):
            ...
            # sample
            sampled, replay_buffer = sample_sequences(
                replay_buffer,
                config.train.batch_size,
                config.train.sequence_steps,
                config.episode_steps,
            )
            (
                sampled_obs_seq,
                sampled_action_seq,
                sampled_next_obs_seq,
                sampled_reward_seq,
                sampled_terminated_seq,
            ) = sampled
            ...
            actor_state, policy_loss = agent.update_actor(
                actor_state, critic_state, sampled_obs_seq, sampled_action_seq
            )
        ...
        def body_fun(carry, _):
            ...
            action, next_lstm_carry = agent.actor.apply(
                actor_state.params, jnp.expand_dims(obs, axis=1), lstm_carry
            )
            action = action.squeeze()
    ...
    def act_episode(key_env, key_obs, params):
        ...
        # initialise lstm carry
        # we are carrying over the hidden state from the current observation to the next
