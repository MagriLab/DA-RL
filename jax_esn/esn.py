...
def closed_loop(params, x0, N_t):
    ...
    (x_final, y_final), (X_preceed, Y_preceed) = jax.lax.scan(
        fn_body, carry0, None, length=N_t
    )
    X = jnp.vstack((x0, X_preceed))
...
def train(params, U_washout, U_train, Y_train):
    ...
    X_train_augmented = jnp.hstack(
        (X_train, params.b_out * jnp.ones((X_train.shape[0], 1)))
    )

    # solve for W_out using linalg solve
    return solve_ridge(X_train_augmented, Y_train, params.tikh)
