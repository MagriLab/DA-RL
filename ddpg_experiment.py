...
flags.DEFINE_bool('log_offline', False, 'Use --log_offline to log the experiment to local.')
flags.DEFINE_bool('save_checkpoints', False, 'Use --save_checkpoints to save intermediate model weigths.')
_CONFIG = config_flags.DEFINE_config_file('config', 'configs/config.py', 'Contains configs to run the experiment')
_WANDB = config_flags.DEFINE_config_file('wandb_config', 'configs/wandb_config.py', 'Contains configs to log to wandb.')
_ENV = config_flags.DEFINE_config_file('env_config', None, 'Contains configs for the environment.')
...
def add_gaussian_noise(key, action, stddev):
    noise = stddev * jax.random.normal(key, shape=action.shape)
    return action + noise
...
def evaluate_KS_for_plotting(env, actor, params, eval_episodes=2):
    ...
    fig, axs = plt.subplots(eval_episodes,3,width_ratios=[1,0.55,1], figsize=(10,3*eval_episodes), constrained_layout=True)

    for i in range(eval_episodes):       
        state, _= env.reset()
        full_state = env.unwrapped.u

        state_arr = np.empty_like(state)
        full_state_arr = np.empty_like(full_state)
        ...
        im = axs[i,0].imshow(full_state_arr.T, extent=[0,len(full_state_arr),x[0],x[-1]], origin='lower', aspect='auto')
        axs[i,0].set_xlabel('t')
        axs[i,0].set_ylabel('x')
        cbar = fig.colorbar(im, ax=[axs[i,0]], location='left')
        cbar.ax.set_title('u')
        axs[i,0].set_title(f"Return={info['episode']['r'][0]:.2f}, Ave. Reward={info['episode']['r'][0]/info['episode']['l'][0]:.2f}")
        ...
        im = axs[i,2].imshow(err, extent=[0,len(full_state_arr),x[0],x[-1]], origin='lower', aspect='auto', cmap='Reds')
...
def run_experiment(config, env, eval_env, agent, wandb_run=None, logs=None, checkpoint_dir=None):
    # random seed for initialization
    key = jax.random.PRNGKey(config.seed)
    
    # initialize networks
    actor_state, critic_state = agent.initial_network_state(env, key)

    # checkpoint the initial weights
    if checkpoint_dir is not None:
        orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
        options = orbax.checkpoint.CheckpointManagerOptions(save_interval_steps=config.total_steps // 5)
    ...
    exploration_stddev = (env.action_space.high-env.action_space.low)*config.train.exploration_stddev
    ...
    while global_step < config.total_steps:
        if global_step < config.learning_starts:
            ...
            action = jnp.clip(action, min=env.action_space.low, max=env.action_space.high)
        ...
        if 'episode' in info: # RecordEpisodeStatistics wrapper from gymnasium
            n_episode += 1
            print(f'Episode {n_episode}, Step={global_step}/{config.total_steps}, Return={info["episode"]["r"][0]}', flush=True)
            # episode statistics to log
            metrics['train']['episode_return'] = info['episode']['r'][0]
            metrics['train']['episode_length'] = info['episode']['l'][0]
            metrics['train']['episode_average_reward'] = info['episode']['r'][0] / info['episode']['l'][0]
        ...
        if global_step >= config.learning_starts and replay_buffer.is_ready(config.train.batch_size):
            sampled_state, sampled_action, sampled_next_state, sampled_reward, sampled_terminated = replay_buffer.sample(config.train.batch_size)
            ...
            actor_state, critic_state = agent.update_target_networks(actor_state, critic_state)
            ...
            if global_step % config.eval_freq == 0:
                ...
                eval_ave_return, eval_ave_reward, eval_ave_last_reward  = evaluate(config, eval_env, agent.actor, actor_state.target_params)
        ...
        if checkpoint_dir is not None:
            checkpoint = {'actor': actor_state, 'critic': critic_state}
            save_args = orbax_utils.save_args_from_target(checkpoint)
            checkpoint_manager.save(global_step, checkpoint, save_kwargs={'save_args': save_args})
...
def main(_):
    ...
    if FLAGS.log_wandb:
        wandb_run = wandb.init(config=config, **FLAGS.wandb_config)
    else:
        wandb_run = None
    ...
    actor_state, critic_state, logs = run_experiment(config, env, eval_env, agent, wandb_run, logs, checkpoint_dir)
    
    # save the final model weights
    orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
    final_checkpoint = {'actor': actor_state, 'critic': critic_state}                                                      
    save_args = orbax_utils.save_args_from_target(final_checkpoint)
    orbax_checkpointer.save(FLAGS.experiment_path / 'final_model', final_checkpoint, save_args=save_args)
