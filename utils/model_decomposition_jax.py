# author: Yaxin Mo, https://github.com/MagriLab/FlowReconstructionFromExperiment/tree/main/flowrec
...
class POD:
    ...
    def pod(
        ...
        if len(q.shape) != 2:
            raise ValueError(
                f"Input q must be of shape [nx,nt], recieved shape {q.shape}"
            )
    ...
    def reconstruct(
        ...
        if grid_shape is not None:
            if np.prod(grid_shape) < rebuildv.size:
                ...
                raise ValueError(
                    f"The provided grid shape implies the reconstructed field has {np.prod(grid_shape)} elements, but the reconstructed field has {rebuildv.size} elements."
                )
